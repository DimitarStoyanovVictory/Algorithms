namespace Exercises_QeenPuzle
{
    ...
    public class EigthQueens
    {
        ...
        private static void UnmarkAllAttackedPositions(int row, int col)
        {
            ...
            RemoveAllChessboardPlaces(row);

            for (int colIndex = col + 1; colIndex < Size; colIndex++)
            {
                if (CanPlaceQeen(row, colIndex))
                {
                    MarkAllAttackedPosition(row, colIndex);
                    PutQueens(row +1);
                }
            }
        }

        private static void RemoveAllChessboardPlaces(int row)
        {
            for (int placeIndex = 0; placeIndex < Size; placeIndex++)
            {
                chessboard[row, placeIndex] = false;
                if (row > 0)
                {
                    chessboard[row - 1, placeIndex] = false;
                }
            }
        }
