namespace Problem06.ConnectedAreasInMatrix
{
    ...
    public class ConnectedAreasInMatrix
    {
        ...
        private static void PrintAreas()
        {
            ...
            Areas.OrderBy(x => x.AreaPoints.Count);

            for (int areaIndex = 0; areaIndex < Areas.Count; areaIndex++)
            {
                Console.WriteLine("Area #{0} at ({1}, {2}), size: {3}", areaIndex + 1, Areas[areaIndex].AreaPoints.First().RowPoint, Areas[areaIndex].AreaPoints.First().ColPoint, Areas.Count);
            }
        }
