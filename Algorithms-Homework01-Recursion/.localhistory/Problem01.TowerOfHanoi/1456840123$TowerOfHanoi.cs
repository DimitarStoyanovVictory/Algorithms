...
public class TowerOfHanoi
{
    ...
    private static void MoveDisks(int bottomDisk, Stack<int> source, Stack<int> destination, Stack<int> spare)
    {
        ...
        {
            if (destination.Count != 0 && source.Count != 0 && source.Peek() > destination.Peek() && spare.Count == 0)
            {
                stepsTaken++;
            ...
            else if ((destination.Count != 0 && source.Count != 0 && source.Peek() > destination.Peek()) || source.Peek() < spare.Peek())
            {
                stepsTaken++;
                spare.Push(source.Pop());
                PrintRods(stepsTaken, spare.Peek());
                MoveDisks(bottomDisk, source, destination, spare);
            } else if (destination.Count != 0 && source.Count != 0 && destination.Peek() < spare.Peek())
            {
                stepsTaken++;
            ...
            if (spare.Peek() < destination.Peek())
            {
                stepsTaken++;
