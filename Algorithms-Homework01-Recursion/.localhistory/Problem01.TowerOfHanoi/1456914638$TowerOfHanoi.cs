...
public class TowerOfHanoi
{
    ...
    private static void MoveDisks(int bottomDisk, Stack<int> source, Stack<int> destination, Stack<int> spare)
    {
        ...
        else
        {
            ...
            if (destination.Count != 0 && source.Count != 0 && spare.Count == 0 && destination.Peek() < source.Peek())
            {
            ...
            else if (destination.Count != 0 && source.Count != 0 && spare.Count != 0 && source.Peek() > destination.Peek() && source.Peek() > spare.Peek())
            {
            ...
            if (source.Count == 0 && spare.Count == 0)
            {
                return;
            }
            if (source.Count == 0)
