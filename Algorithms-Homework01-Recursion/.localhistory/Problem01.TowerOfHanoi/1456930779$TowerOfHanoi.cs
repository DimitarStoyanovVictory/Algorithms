...
public class TowerOfHanoi
{
    ...
    private static void MoveDisks(int bottomDisk, Stack<int> source, Stack<int> destination, Stack<int> spare)
    {
        ...
        else
        {
            ...
            if (source.Count != 0 && destination.Count != 0 && spare.Count != 0 && source.Peek() > destination.Peek() && source.Peek() > spare.Peek())
            {
                
            }
            else if (destination.Count != 0 && destination.Peek() > source.Peek() && (spare.Count == 0))
            {
