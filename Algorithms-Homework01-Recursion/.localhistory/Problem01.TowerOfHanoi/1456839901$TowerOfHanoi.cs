...
public class TowerOfHanoi
{
    ...
    private static void MoveDisks(int bottomDisk, Stack<int> source, Stack<int> destination, Stack<int> spare)
    {
        ...
        {
            if ((destination.Count != 0 && source.Count != 0 && source.Peek() > destination.Peek())
            {
                stepsTaken++;
            ...
            if ((destination.Count != 0 && source.Count != 0 && source.Peek() > destination.Peek()) || source.Peek() < spare.Peek())
            {
                stepsTaken++;
                spare.Push(source.Pop());
                PrintRods(stepsTaken, spare.Peek());
            ...
            if (destination.Count != 0 && source.Count != 0 && destination.Peek() < spare.Peek())
            {
                stepsTaken++;
