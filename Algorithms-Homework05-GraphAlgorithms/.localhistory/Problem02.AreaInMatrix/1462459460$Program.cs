    ...
    class Program
    {
        static void Main()
        {
            ...
            for (int row = 0; row < inputMatrix.GetLength(0); row++)
            {
                ...
                {
                    if (IsValid(row, col + 1) && inputMatrix[row, col] == inputMatrix[row, col + 1])
                    {
                        visited[row, col + 1] = true;
                        AddArea(ref areas, inputMatrix[row, col + 1]);
                    }
                    else if(IsValid(row - 1, col) && inputMatrix[row, col] == inputMatrix[row - 1, col])
                    {
                        visited[row - 1, col] = true;
                        AddArea(ref areas, inputMatrix[row + 1, col]);
                    }
                    else if (IsValid(row, col - 1) && inputMatrix[row, col] == inputMatrix[row, col - 1])
                    {
                        visited[row, col - 1] = true;
                        AddArea(ref areas, inputMatrix[row + 1, col]);
                    }
