namespace Problem02.AreaInMatrix
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            var tempArea = new Dictionary<char, int>();
            int tempRow = 0;
            int tempCol = 0;
                ...
                for (int col = 0; col < inputMatrix.GetLength(1); col++)
                {
                    ...
                    while (true)
                    {
                        if (IsValid(tempRow - 1, tempCol, inputMatrix) && !visited[tempRow - 1, tempCol] && inputMatrix[tempRow, tempCol] == inputMatrix[tempRow - 1, tempCol])
                        {
                            ...
                            if (!tempArea.Keys.Contains(inputMatrix[tempRow, tempCol]))
                            {
                                tempArea[inputMatrix[tempRow, tempCol]] = 1;
                            }
                            else if (visited[tempRow - 1, tempCol])
                            {
