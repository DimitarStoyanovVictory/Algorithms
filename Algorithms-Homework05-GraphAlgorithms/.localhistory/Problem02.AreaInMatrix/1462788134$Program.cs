            ...
            for (int row = 0; row < inputMatrix.GetLength(0); row++)
            {
                for (int col = 0; col < inputMatrix.GetLength(1); col++)
                {
                    ...
                    while (true)
                    {
                        ...
                        else if (IsValid(row, col - 1, inputMatrix) && !visited[row, col - 1] && inputMatrix[row, col] == inputMatrix[row, col - 1])
                        {
                            visited[row, col - 1] = true;
                            AddArea(ref areas, inputMatrix[row, col - 1]);
                            col--;
                        }
                        else if (IsValid(row + 1, col, inputMatrix) && !visited[row + 1, col] && inputMatrix[row, col] == inputMatrix[row + 1, col])
                        {
