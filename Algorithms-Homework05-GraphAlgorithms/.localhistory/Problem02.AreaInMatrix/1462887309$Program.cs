    ...
    class Program
    {
        ...
        private static bool SingleElement(char[,] inputMatrix, int tempRow, int tempCol)
        {
            if (IsValid(tempRow - 1, tempCol, inputMatrix) 
                && inputMatrix[tempRow - 1, tempCol] != inputMatrix[tempRow, tempCol]
                && IsValid(tempRow + 1, tempCol, inputMatrix)
                && inputMatrix[tempRow + 1, tempCol] != inputMatrix[tempRow, tempCol]
                && IsValid(tempRow, tempCol + 1, inputMatrix) && inputMatrix[tempRow, tempCol + 1] != inputMatrix[tempRow, tempCol])
            {
                return true;
            }
