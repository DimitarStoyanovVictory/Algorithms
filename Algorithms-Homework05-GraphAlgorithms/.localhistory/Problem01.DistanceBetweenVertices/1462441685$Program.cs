            ...
            { 
                ...
                else
                {
                    ...
                    foreach (var element in graph[distanceToFind[indexDistanceToFind][0]])
                    {
                        ...
                        MakeSequance(ref results, graph, lastElement, visited, currentElement, count, resultCount);
                    }
                }

                Console.WriteLine("{" + $"{distanceToFind[indexDistanceToFind][0]}, {distanceToFind[indexDistanceToFind][1]}" + @"} -> " + $"{results.Min()}]");
            }
        }

        private static void MakeSequance(ref List<int> results, List<List<int>> graph, int lastElement, bool[] visited, int currentElement, int count, int resultCount)
        {
            while (true)
            {
