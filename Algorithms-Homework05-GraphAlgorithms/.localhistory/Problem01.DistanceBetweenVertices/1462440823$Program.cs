namespace Problem01.DistanceBetweenVertices
{
    ...
    class Program
    {
        ...
        private static void MakeSequance(List<int> results, List<List<int>> graph, int lastElement, bool[] visited, int currentElement, int count, int resultCount)
        {
            while (true)
            {
                ...
                if (graph[currentElement].Count > 1)
                {
                    foreach (var secondElement in graph[currentElement])
                    {
                        if (!visited[secondElement])
                        {
                            ...
                            MakeSequance(graph, results, lastElement, visited, currentElement, count, resultCount);
