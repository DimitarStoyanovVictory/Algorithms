namespace Problem01.DistanceBetweenVertices
{
    ...
    class Program
    {
            ...
            { 
                ...
                else
                {
                    ...
                    foreach (var element in graph[distanceToFind[indexDistanceToFind][0]])
                    {
                        ...
                        MakeSequance(graph, lastElement, ref visited, ref currentElement, ref count, ref resultCount);
                    }
                }
        ...
        private static void MakeSequance(List<List<int>> graph, int lastElement, ref bool[] visited, ref int currentElement, ref int count, ref int resultCount)
        {
            while (true)
            {
                MakeSequance(graph, lastElement, ref visited, ref currentElement, ref count);
                ...
                if (graph[currentElement].Count > 1)
                {
                    foreach (var secondElement in graph[currentElement])
                    {
                        ...
                        {
                            currentElement = graph[currentElement][secondElement];
