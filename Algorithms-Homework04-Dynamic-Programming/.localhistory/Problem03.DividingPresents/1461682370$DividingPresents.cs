    ...
    class DividingPresents
    {
        static void Main()
        {
            ...
            for (int indexFirst = 1; indexFirst < list.Count - 1; indexFirst++)
            {
                ...
                {
                    for (int indexThird = 0; indexThird < list.Count; indexThird++)
                    {
                        list.GetRange(indexSecond, indexFirst - indexThird).ForEach(x => alanSum += x);
                        list.GetRange(indexThird, indexThird).ForEach(x => alanSum += x);
