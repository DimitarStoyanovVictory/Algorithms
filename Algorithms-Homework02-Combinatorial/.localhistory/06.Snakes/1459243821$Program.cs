namespace Snakes
{
    ...
    public class Program
    {
        ...
        private static void MovingCombinations(int i, int n)
        {
            if (i > 1)
            {
                _result = _result.GetRange(0, n);
                _result.RemoveRange(n - i, i + 1);
            }
        ...
        {
            if (_result.Count > 2 && _result[_result.Count - 1] == 'R' && direction == 'L')
            {
                _result.Add('R');
                return;
            }
            else if (_result.Count > 2 && _result[_result.Count - 1] == 'U' && (direction == 'D' || direction == 'U'))
            {
                _result.Add('R');
                return;
            }
            else if (_result.Count > 2 && _result[_result.Count - 1] == 'D' && direction == 'U')
            {
                _result.Add('R');
                return;
            }
            else if (_result.Count > 2 && _result[_result.Count - 1] == 'R' && direction == 'U')
            {
                _result.Add('R');
                return;
            }
